...
# Seed and Node Delivery Split

The overall delivery must contain two clearly separate, independently extractable ZIP artifacts:

- `seed/qv2134_seed.zip`
- `node/qv2134_node.zip`

Both contain the same QIKVRT Node Core and the same GitHub-compatible API. The only intended operational difference is the runtime role profile:

- Seed role: publishes/anchors the seed manifest and release evidence.
- Normal node role: onboards through the seed graph and participates through the same API.

This split is a packaging and runtime-profile split, not a source-code fork and not an API fork.
