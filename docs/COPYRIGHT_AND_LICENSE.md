...
# Copyright, License and Header/Footer Policy

This repository makes author, rights and license information visible at repository level, source-file level and document level.
...
1. C source and header files must contain a QIKVRT artifact header.
2. Shell, CMD and PowerShell scripts must contain a QIKVRT artifact header.
3. Markdown documents must contain a QIKVRT metadata header and a license footer.
4. YAML/OpenAPI documents must contain a QIKVRT comment header where the format allows comments.
5. JSON/JSONL files remain parseable and therefore carry license/provenance through explicit metadata files and gates, not comments.

## Delivery Rule

Seed repository ZIP and Node repository ZIP must be present as separate, clearly named independent ZIP artifacts inside the overall delivery.

## No False Pass

Missing rights, license headers or delivery split evidence blocks final acceptance.
