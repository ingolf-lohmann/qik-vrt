...
V45.12 behavior: if `origin/main` exists, the package uses `origin/main` as canonical base, restores the extracted QIKVRT content as an overlay, commits that overlay locally, verifies `origin/main` is an ancestor of `HEAD`, and only then attempts push/release.
