...
set -eu
# The 2.13.4P node deploy path must not create/upload GitHub releases locally.
...
grep -q 'NODE_GIT_PUSH_INTERACTIVE_CREDENTIAL_HANG' release/QV2134P_NODE_GIT_IDEMPOTENT_TAG_PUSH_REPAIR_REF.md
grep -q 'NODE_GIT_ARCHIVE_PATH_ARRAY_DRIVE_PARSE_FAILURE' release/QV2134P_NODE_GIT_IDEMPOTENT_TAG_PUSH_REPAIR_REF.md
grep -q "'remote', 'add', 'origin'" tools/gh_deploy.ps1
grep -q "'remote', 'set-url', 'origin'" tools/gh_deploy.ps1
grep -q 'remote get-url origin' release/QV2134P_NODE_GIT_IDEMPOTENT_TAG_PUSH_REPAIR_REF.md
...
grep -q 'NODE_LOCAL_TAG_EXISTS_IDEMPOTENCE_BLOCKS_PUSH_AFTER_PARTIAL_RUN' release/QV2134P_NODE_GIT_IDEMPOTENT_TAG_PUSH_REPAIR_REF.md
