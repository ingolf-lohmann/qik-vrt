...
P = ROOT / "tools/qikvrt_content_disposition_status_after_batch_002_acceptance_compat.py"
...
class T(unittest.TestCase):
    ...
    def test_post_acceptance_overlay_is_deterministic(self):
        ...
        self.assertEqual(corpus["active_batch"]["state"], "READY")
    ...
    def test_terminal_history_is_not_rewritten(self):
        _, _, _, receipt, _ = m.historical_projection()
        old = "CREATE_CORRECTED_CANDIDATES_AND_RETURN_TO_OWNER_FOR_BATCH_002"
        self.assertEqual(receipt["next_deterministic_effect"], old)
        self.assertEqual(self.queue["next_deterministic_effect"], old)
        self.assertEqual(self.index["next_deterministic_effect"], old)
        self.assertEqual(self.union["next_deterministic_effect"], old)
        self.assertTrue(
    ...
    def test_open_subject_tamper_blocks(self):
