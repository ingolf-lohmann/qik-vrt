param([Parameter(Mandatory=$true)][string]$EvidenceJson)
...
if ($ev.evidence_version -ne "QIKVRT_GITHUB_REMOTE_EFFECT_EVIDENCE_V45_12") { Fail "wrong evidence_version" }
...
if ($ev.no_force_tag_update -ne $true) { Fail "no_force_tag_update is not true" }
if ($ev.tag_force_updated -ne $false) { Fail "tag_force_updated is not false" }
if ($ev.asset_clobbered -ne $false) { Fail "asset_clobbered is not false" }
...
if ($ev.remote_tag_sha -notmatch '^[0-9a-fA-F]{40,64}$') { Fail "remote_tag_sha invalid" }
...
if ($ev.local_commit_sha.ToString().ToLowerInvariant() -ne $ev.remote_tag_sha.ToString().ToLowerInvariant()) { Fail "remote_tag_sha does not match local_commit_sha" }
if ($ev.pushed_ref_verified -ne $true) { Fail "pushed_ref_verified is not true" }
if ($ev.tag_ref_verified -ne $true) { Fail "tag_ref_verified is not true" }
...
Write-QikvrtPass "GitHub remote effect evidence freeze gate ok"
exit 0
