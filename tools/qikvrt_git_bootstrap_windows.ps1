param(
  ...
  [string]$RemoteUrl = "",
  [string]$CommitMessage = "QIKVRT V45.11 remote-based overlay bootstrap before real GitHub release",
...
if (!$inside) {
  Write-QikvrtContinue "no .git repository found; initializing local Git repository"
  & git init
  if ($LASTEXITCODE -ne 0) { Write-QikvrtBlock "git init failed"; exit 1 }
} else {
  Write-QikvrtPass "existing Git repository detected"
}
...
if ($remoteProbe.ExitCode -ne 0 -or [string]::IsNullOrWhiteSpace($remoteProbe.Stdout.Trim())) {
  if ([string]::IsNullOrWhiteSpace($normalized)) {
    Write-QikvrtBlock "git origin missing; provide GitHub repository URL or OWNER/REPO in the wrapper prompt or set QIKVRT_GITHUB_REMOTE_URL"
    exit 30
  }
  ...
  $origin = $remoteProbe.Stdout.Trim()
  Write-QikvrtPass "git origin present: $origin"
}

$remoteExists = Test-QikvrtRemoteBranchExists -RemoteName "origin" -Branch $TargetBranch
if ($remoteExists) {
  Write-QikvrtContinue "origin/$TargetBranch exists; using remote branch as canonical base before overlay commit"
  $overlayPath = Join-Path $env:TEMP ("qikvrt_v45_11_overlay_" + [guid]::NewGuid().ToString("N"))
  try {
    ...
    Write-QikvrtPass "QIKVRT overlay restored on top of origin/$TargetBranch"
  } finally {
    if (Test-Path -LiteralPath $overlayPath) { Remove-Item -LiteralPath $overlayPath -Recurse -Force }
  }
} else {
...
if ($headProbe.ExitCode -ne 0) {
  ...
  if ($LASTEXITCODE -ne 0) { Write-QikvrtBlock "initial git commit failed after repository-local identity bootstrap"; exit 1 }
  Write-QikvrtPass "initial local Git commit created"
} elseif ($status) {
  & git commit -m $CommitMessage
  if ($LASTEXITCODE -ne 0) { Write-QikvrtBlock "remote-based overlay commit failed after repository-local identity bootstrap"; exit 1 }
  Write-QikvrtPass "remote-based QIKVRT overlay commit created"
} else {
  Write-QikvrtPass "local Git working tree already clean after remote base overlay"
}

if ($remoteExists) {
  $remoteRef = "origin/$TargetBranch"
  if (Test-QikvrtRemoteIsAncestorOfHead -RemoteRef $remoteRef) {
    Write-QikvrtPass "$remoteRef is ancestor of HEAD; push can be fast-forward"
  } else {
    Write-QikvrtBlock "$remoteRef is not ancestor of HEAD after overlay bootstrap; refusing divergent push"
    exit 41
  }
}
