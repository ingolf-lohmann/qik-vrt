...
function Push-BranchBestEffort([string]$Branch,[string]$Tag,[string]$AuthHeader,[hashtable]$PushEnv,[int]$PushTimeout){
  ...
  if (Test-GitNonFastForwardFailure $text) {
    ...
    Add-DeployResult 'GIT_PUSH_BRANCH_FALLBACK' 'BLOCK' ([string]$fb.text)
...
function New-GitArchiveAsset([string]$Tag,[string]$Role){
  $asset = [string](Join-Path $RuntimeDir ('qv2134_' + $Role + '.zip'))
  if (Test-Path -LiteralPath $asset) { Remove-Item -LiteralPath $asset -Force }
  $null = Invoke-GitChecked @('-C', $RepoRoot, 'archive', '--format=zip', ('--output=' + $asset), $Tag) 'DEPLOY_ASSET_GIT_ARCHIVE' $asset
  Add-DeployResult 'DEPLOY_STAGING_SELF_COPY_PREVENTION' 'PASS' '.gitattributes export-ignore excludes qikvrt/runtime/deploy, qikvrt/runtime/bootstrap, qikvrt/runtime/win_acceptance, qikvrt/runtime/setup, package_staging, build, and git metadata'
  return [string]$asset
}
# QIKVRT V2.13.4P: no AskPass token file is written. Git push uses a non-persistent http.extraheader for this process only.
try {
  $target = Resolve-OwnerRepo
  $role = RoleName
  if ($role -ne 'node') { Add-DeployResult 'NODE_ONLY_VARIANT' 'BLOCK' ('role=' + $role + '; this 2.13.4P package is node-only'); Write-DeployLogsAndExit 3 }
  ...
  Add-DeployResult 'GIT_BRANCH_FALLBACK_GATE' 'PASS' 'non-fast-forward primary branch push falls back to qikvrt-node/<tag>; auth/permission failures still BLOCK'
  $tag = if ($env:QIKVRT_RELEASE_TAG) { [string]$env:QIKVRT_RELEASE_TAG } else { 'v2.13.4-node' }
  $branch = Ensure-GitRepository $target
  CommitNodeState $tag
  PrepareReleaseTag $tag
  $asset = New-GitArchiveAsset $tag $role
  $hash = Sha256 $asset
  Add-DeployResult 'DEPLOY_ASSET_PACKAGE' 'PASS' $asset
  Add-DeployResult 'DEPLOY_ASSET_SHA256' 'PASS' $hash
  Add-DeployResult 'LOCAL_RELEASE_REST_API' 'PASS' 'disabled; local deploy uses git push only; GitHub Actions in target repo creates/releases asset'
