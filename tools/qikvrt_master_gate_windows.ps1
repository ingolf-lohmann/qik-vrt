param([switch]$RequireGithubRelease,[string]$EvidenceJson = "")
$ErrorActionPreference = "Stop"
. (Join-Path $PSScriptRoot "qikvrt_common_windows.ps1")
if ($RequireGithubRelease) {
  if ([string]::IsNullOrWhiteSpace($EvidenceJson)) { Write-QikvrtBlock "RequireGithubRelease set but EvidenceJson missing"; exit 1 }
  ...
  Write-QikvrtPass "master gate with real GitHub release evidence-freeze ok"
  exit 0
}
