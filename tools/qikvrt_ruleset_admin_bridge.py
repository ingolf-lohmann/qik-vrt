...
def execute(event,env,reconciler,api=request,signer=sign_jwt,clock=time.time):
    ...
    try:
        validate_event(event,env); require(bool(read_token),"READ_TOKEN_MISSING"); receipt.update(bind(api,read_token,head)); policy=reconciler.load_policy(); path="/repos/"+AUTHORITY+"/rulesets/"+str(RULESET); before=reconciler.evaluate(api("GET",path,read_token),policy); receipt["pre_ruleset_state"]=before["state"]
...
def main():
    ...
    except Exception: result={"schema":"qikvrt_ruleset_admin_bridge_v3","state":"HOLD_UNVERIFIED","first_blocker":"ADAPTER_PREFLIGHT_FAILED","mutation":"NONE","effect_observed":False,"evidence_transfer":False,"effect_ack_done":False}
