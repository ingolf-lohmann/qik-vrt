...
def complete_ruleset_snapshot(snapshot):
    """GitHub redacts bypass_actors without ruleset write access; absent is unknown."""
    return isinstance(snapshot.get("bypass_actors"), list)
...
def execute(event,env,reconciler,api=request,signer=sign_jwt,clock=time.time):
    ...
    receipt = {
        "schema": "qikvrt_ruleset_admin_bridge_v5", "repository": AUTHORITY,
        "carrier_repository": CARRIER, "carrier_head": head,
        ...
        "ruleset_current": False, "ruleset_comparison": "UNKNOWN", "full_readback": False,
        "continuation_required": True, "review_submission": False, "source_promotion": False,
        "deployment": False, "evidence_transfer": False, "effect_ack_done": False,
    ...
    try:
        ...
        binding = bind(api, read_token, head)
        receipt.update(binding)
        require(env.get("GITHUB_EVENT_NAME") != "push" or event.get("before") == binding["carrier_parent"], "PUSH_PARENT_MISMATCH")
        policy = reconciler.load_policy()
        path = "/repos/" + AUTHORITY + "/rulesets/" + str(RULESET)
        public_before = api("GET", path, read_token)
        before = reconciler.evaluate(public_before, policy)
        complete_before = complete_ruleset_snapshot(public_before)
        receipt["pre_ruleset_complete"] = complete_before
        receipt["pre_ruleset_state"] = before["state"] if complete_before or before["state"] != "CURRENT" else "VISIBLE_PROJECTION_CURRENT"
        needs_reconcile = before["state"] != "CURRENT"
        if needs_reconcile or not complete_before:
            # The existing, narrowly scoped token also supplies visibility of the
            # protected bypass list. No new credential name or permission is added.
            ...
            receipt.update(installation_id=installation, token_scope_verified=True)
            if not complete_before:
                scoped_before = api("GET", path, minted)
                require(complete_ruleset_snapshot(scoped_before), "RULESET_BYPASS_VISIBILITY_MISSING")
                full_before = reconciler.evaluate(scoped_before, policy)
                receipt["pre_full_ruleset_state"] = full_before["state"]
                needs_reconcile = full_before["state"] != "CURRENT"
            require(bind(api, read_token, head)["carrier_tree"] == receipt["carrier_tree"], "CARRIER_TREE_DRIFT")
            if needs_reconcile:
        ...
        # Always perform a new GET. Neither the PUT response nor the full
        # preflight snapshot is evidence of the post-operation state.
        public_after = api("GET", path, read_token)
        require(reconciler.evaluate(public_after, policy)["state"] == "CURRENT", "INDEPENDENT_RULESET_READBACK_NOT_CURRENT")
        observed = public_after
        readback_credential = "READ_TOKEN"
        if not complete_ruleset_snapshot(observed):
            require(bool(minted), "RULESET_BYPASS_VISIBILITY_MISSING")
            observed = api("GET", path, minted)
            readback_credential = "SCOPED_APP_TOKEN_GET_ONLY"
        require(complete_ruleset_snapshot(observed), "RULESET_BYPASS_VISIBILITY_MISSING")
        final = reconciler.evaluate(observed, policy)
        require(final["state"] == "CURRENT", "INDEPENDENT_RULESET_READBACK_NOT_CURRENT")
        require(final["pre_state_sha256"] == final["desired_state_sha256"], "INDEPENDENT_RULESET_DIGEST_MISMATCH")
        require(bind(api, read_token, head)["carrier_tree"] == receipt["carrier_tree"], "POST_EFFECT_SUBJECT_DRIFT")
        receipt.update(
            state="RULESET_CURRENT", ruleset_current=True, ruleset_comparison="MATCH",
            full_readback=True, effect_observed=receipt["mutation"] == "PUT", first_blocker=None,
            next_action="REOBSERVE_EXACT_CARRIER_VALIDATION", ruleset_digest=final["pre_state_sha256"],
            desired_ruleset_digest=final["desired_state_sha256"], readback_credential=readback_credential,
            readback_independence="SEPARATE_GET_NOT_SEPARATE_PRINCIPAL",
        )
    except Hold as exc:
        receipt.update(first_blocker=str(exc), next_action="BIND_EXISTING_RULESET_APP_CONFIGURATION_IN_CARRIER_ACTIONS" if str(exc) == "RULESET_APP_CONFIGURATION_MISSING" else "REPAIR_BOUND_ADMINISTRATIVE_CARRIER")
    except Exception:
        ...
        if minted:
            try:
                ...
                receipt.update(token_revoked=False, cleanup_blocker="TOKEN_REVOCATION_UNCONFIRMED", state="HOLD_UNVERIFIED", first_blocker=receipt.get("first_blocker") or "TOKEN_REVOCATION_UNCONFIRMED")
    return receipt
...
def main():
    ...
    except Exception: result={"schema":"qikvrt_ruleset_admin_bridge_v5","state":"HOLD_UNVERIFIED","first_blocker":"ADAPTER_PREFLIGHT_FAILED","mutation":"NONE","effect_observed":False,"evidence_transfer":False,"effect_ack_done":False}
