...
def evaluate_promotion(snapshot: Mapping[str, Any]) -> dict[str, Any]:
    ...
    verifier = latest.get(EXACT_HEAD_VERIFIER)
    if verifier is None:
        return _blocked(
            snapshot,
            "EXACT_HEAD_VERIFIER_MISSING",
            "the trusted repository-dispatch verifier is absent",
        )
    if verifier.get("event") != "repository_dispatch":
        return _blocked(
            snapshot,
            "EXACT_HEAD_VERIFIER_UNTRUSTED_EVENT",
            "the exact-head verifier was not invoked by repository dispatch",
        )
    if verifier.get("status") != "completed" or verifier.get("conclusion") not in SUCCESS_CONCLUSIONS:
        return _blocked(
            snapshot,
            "EXACT_HEAD_VERIFIER_NOT_GREEN",
            "the exact-head verifier is not terminal success",
        )
