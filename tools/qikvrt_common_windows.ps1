...
function Normalize-QikvrtPathArg {
  ...
  try {
    if (Test-Path -LiteralPath $p) {
      return (Trim-QikvrtTrailingSeparator ((Resolve-Path -LiteralPath $p).Path))
    }
    if ($MustExist) { throw "path does not exist: $p" }
    $full = [System.IO.Path]::GetFullPath($p)
    return (Trim-QikvrtTrailingSeparator $full)
  } catch {
    if ($MustExist) { throw }
    try {
      $full2 = [System.IO.Path]::GetFullPath($p)
      return (Trim-QikvrtTrailingSeparator $full2)
    } catch {
      throw "invalid path argument: $p"
    }
  }
}

function New-QikvrtDirectoryForFile([string]$Path) {
  $dir = Split-Path -Parent $Path
  if (![string]::IsNullOrWhiteSpace($dir)) { New-Item -ItemType Directory -Force -Path $dir | Out-Null }
}
function Write-QikvrtJson([string]$Path, $Object) {
  New-QikvrtDirectoryForFile $Path
  ($Object | ConvertTo-Json -Depth 20) | Set-Content -LiteralPath $Path -Encoding UTF8
}
function Read-QikvrtJson([string]$Path) {
  if (!(Test-Path -LiteralPath $Path)) { throw "JSON file missing: $Path" }
  return (Get-Content -LiteralPath $Path -Raw -Encoding UTF8 | ConvertFrom-Json)
}
function Assert-QikvrtCommand([string]$Name) {
  $cmd = Get-Command $Name -ErrorAction SilentlyContinue
  if ($null -eq $cmd) { throw "required command missing: $Name" }
  return $cmd.Source
}
function Get-QikvrtSha256([string]$Path) {
  if (!(Test-Path -LiteralPath $Path)) { throw "file missing for sha256: $Path" }
...
function Join-QikvrtCommandLineArguments([AllowNull()][object[]]$Arguments) {
  if ($null -eq $Arguments) { return '' }
  $parts = @()
  foreach ($a in $Arguments) { $parts += (ConvertTo-QikvrtCommandLineArgument $a) }
  return ($parts -join ' ')
}
function Invoke-QikvrtCapture([string]$FilePath, [AllowNull()][object[]]$Arguments) {
  ...
  if ($null -eq $p) { throw "failed to start process: $FilePath" }
...
function Copy-QikvrtWorktreeOverlayToTemp {
  param(
  ...
  Get-ChildItem -LiteralPath $RepoPath -Force | Where-Object { $_.Name -ne ".git" } | ForEach-Object {
    Copy-Item -LiteralPath $_.FullName -Destination $OverlayPath -Recurse -Force
  }
}
function Clear-QikvrtWorktreeExceptGit {
  param([Parameter(Mandatory=$true)][string]$RepoPath)
  Get-ChildItem -LiteralPath $RepoPath -Force | Where-Object { $_.Name -ne ".git" } | ForEach-Object {
    Remove-Item -LiteralPath $_.FullName -Recurse -Force
  }
}
function Restore-QikvrtWorktreeOverlayFromTemp {
  param(
    [Parameter(Mandatory=$true)][string]$OverlayPath,
    [Parameter(Mandatory=$true)][string]$RepoPath
  )
  Get-ChildItem -LiteralPath $OverlayPath -Force | ForEach-Object {
    Copy-Item -LiteralPath $_.FullName -Destination $RepoPath -Recurse -Force
  }
}
function Test-QikvrtRemoteBranchExists {
  param([string]$RemoteName = "origin", [string]$Branch = "main")
...
function Test-QikvrtRemoteIsAncestorOfHead {
  param([string]$RemoteRef)
  $probe = Invoke-QikvrtCapture "git" @("merge-base", "--is-ancestor", $RemoteRef, "HEAD")
  return ($probe.ExitCode -eq 0)
}

function Get-QikvrtGitConfigValue([string]$Key) {
  $probe = Invoke-QikvrtCapture "git" @("config", "--local", "--get", $Key)
  if ($probe.ExitCode -eq 0) { return $probe.Stdout.Trim() }
  return ""
}
function Set-QikvrtGitConfigIfMissing {
  param(
  ...
  if ([string]::IsNullOrWhiteSpace($current)) {
    ...
    if ($LASTEXITCODE -ne 0) { throw "git config --local $Key failed" }
    Write-QikvrtPass "local git config set: $Key"
  } else {
    Write-QikvrtPass "local git config present: $Key"
  }
}
function Ensure-QikvrtLocalGitIdentity {
  param(
...
function Normalize-QikvrtGitHubRemote([string]$RemoteUrlOrSlug) {
  ...
  if ($v -match '^https://github\.com/[^/]+/[^/]+(\.git)?$') {
    if ($v.EndsWith('.git')) { return $v } else { return ($v + '.git') }
  }
