...
.PHONY: failure-transition-contract
failure-transition-contract:
	PYTHONDONTWRITEBYTECODE=1 PYTHONNOUSERSITE=1 $(PYTHON) -B -m unittest -v tests.test_qikvrt_failure_transition

test: failure-transition-contract
