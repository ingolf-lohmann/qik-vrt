...
Geprüfte lokale Gates:

```text
Python-Syntaxprüfung
Shell-Syntaxprüfung
Shell-Ausführungsbit
Hauptskript---help-Läufe
PowerShell-Syntaxprüfung soweit Umgebung `pwsh` bereitstellt
Exit-Code-Propagation über Wrapper
```

Statusgrenze:

Windows- und macOS-Realmaschine sind in dieser Umgebung nicht tatsächlich ausgeführt. Das Paket enthält jedoch die zielplattformgerechten Launcher und erzwingt die lokal verfügbare Syntax-/Help-/Exit-Code-Prüfung. Externes DONE für Windows/macOS ist erst nach echtem Lauf auf diesen Zielplattformen zulässig.
