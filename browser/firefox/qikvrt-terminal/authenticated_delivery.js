(() => {
  ...
  const REQUESTS = Object.freeze({
    ...
    openai_chatgpt: {
      id: "OPENAI_CHATGPT_WORK_CANCELLATION_V1",
      path: "state/delivery/requests/OPENAI_CHATGPT_WORK_CANCELLATION_V1.json",
      operation: "AUTHENTICATED_OPENAI_SUBSCRIPTION_CANCELLATION"
    }
  });
  ...
  function platformForHost(hostname) {
    ...
    if (host === "chatgpt.com" || host.endsWith(".chatgpt.com")) return "openai_chatgpt";
    return null;
  }
  ...
  function extractOpenAICancellationReadback(text) {
    const normalized = String(text || "").replace(/\s+/g, " ").trim();
    const cancelled = /(?:subscription|plan)[^.]{0,120}(?:cancelled|canceled|will end|ends on|expires)/i.test(normalized) || /(?:cancelled|canceled)[^.]{0,120}(?:subscription|plan)/i.test(normalized);
    const dateMatch = normalized.match(/(?:ends on|expires|effective)[^A-Za-z0-9]{0,12}([A-Za-z]{3,9}\s+\d{1,2},?\s+\d{4}|\d{4}-\d{2}-\d{2})/i);
    return {subscription: "ChatGPT Work", cancellation_state: cancelled ? "CANCELLED_OR_SCHEDULED_TO_END" : null, effective_end_or_termination_date: dateMatch ? dateMatch[1] : null};
  }
  ...
  async function reobservePending() {
    ...
    const observed = platform === "arxiv" ? extractArxivReadback(text) : platform === "wikipedia" ? extractWikipediaReadback(text) : extractOpenAICancellationReadback(text);
    const receipt = {
      ...
      observed,
      authoritative_subject_observed: platform === "arxiv" ? Boolean(observed.submission_id && observed.submission_status) : platform === "wikipedia" ? Boolean(observed.revision_id) : Boolean(observed.subscription === "ChatGPT Work" && observed.cancellation_state),
      completion_claims: {PASS: false, FINAL_PASS: false, EFFECT_ACK_DONE: false}
    };
