...
# Windows Compiler Bootstrap Repair Reference v2.11.3

Observed external Windows result for qv2112w:

- Windows runner started: PASS
- root files, hashes, JSON/JSONL: PASS
- live GitHub raw manifest fetch: PASS
- winget present and LLVM install attempted: PASS
- compiler not found after PATH refresh: BLOCK
- C build/selftests: BLOCK

Repair in qv2113w:

- add known-path compiler discovery after winget/choco
- add current-process PATH insertion for discovered compiler directories
- execute compilers by full path rather than relying only on PATH
- support Visual Studio `VsDevCmd.bat` candidates
- attempt Visual Studio Build Tools C++ workload via winget if no buildable compiler candidate exists
- preserve `NO_FALSE_PASS`: if no executable verifier is built, C selftests remain BLOCK
