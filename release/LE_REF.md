...
# V2.11 Live Evidence Reference Result

Status: `LOCAL_EVIDENCE_CLOSURE_PASS_WITH_EXTERNAL_LIVE_FETCH_PENDING`.

- GitHub seed web visibility: PASS by web evidence.
- Raw manifest reference: PASS by web/reference evidence.
- REST/TCPIP manifest reference: PASS by web/reference evidence.
- Local C/POSIX live fetch from assistant sandbox: DNS_BLOCK_RECORDED.
- External C/POSIX live fetch: REQUIRED for full live acceptance.
- Final live pass without external fetch: BLOCKED.

## Assistant sandbox execution note

`tools/gh_live_fetch.sh .` was executed in the assistant sandbox and returned curl exit code 6: `Could not resolve host: raw.githubusercontent.com`. This is recorded as `SANDBOX_DNS_BLOCK_RECORDED`, not as a live-pass. External runner execution remains required.
