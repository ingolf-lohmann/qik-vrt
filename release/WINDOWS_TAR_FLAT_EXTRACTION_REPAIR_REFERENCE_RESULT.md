...
# V2.10.3 Windows tar flat extraction repair reference

Input evidence: `archiv_report(3).tsv`
...
```text
ZIP_WRAPPER_DIRECTORY_NOT_ACCEPTANCE_RUNNER_COMPATIBLE
```

Repair:

```text
FLAT_ZIP_ROOT_LAYOUT = REQUIRED
NO_WRAPPER_ONLY_EXTRACTION = PASS
README_AT_EXTRACTION_ROOT = PASS
MAKEFILE_AT_EXTRACTION_ROOT = PASS
SHA256SUMS_AT_EXTRACTION_ROOT = PASS
RUNTIME_ROOT_LAYOUT_SELFTEST = PASS
```

This result is a packaging/layout repair. It does not modify the V2.10 GitHub seed-discovery trust boundary.
