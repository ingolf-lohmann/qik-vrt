...
# Windows StrictMode Array Fix Reference v2.11.5

External qv2114w evidence:

- Root files: PASS
- SHA256SUMS: PASS
- JSON/JSONL: PASS
- Live GitHub fetch: PASS
- Failure: PowerShell 5.1 StrictMode at `tools\win_acceptance.ps1:223`
- Error: property `Length` not found for singleton object

Repair in qv2115w:

- `To-ObjectArray` returns an explicitly array-wrapped result.
- `Get-ObjectArrayCount` added.
- Candidate checks no longer call `.Length` on potential singleton compiler objects.
- The previous `.Count`/`.Length` StrictMode failure class is now covered.

Acceptance boundary:

No final Windows C selftest PASS is claimed without a new external Windows runner result.
